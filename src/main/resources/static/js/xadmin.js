;!function (win) {
    ...
    Xadmin.prototype.add_tab = function (title, url, is_refresh) {
        ...
        if (!['/druid/','/swagger-ui/'].includes(url)){
    ...
    Xadmin.prototype.page_loading = function(status){
        // console.log($("#loading",top.window.document))
        if (status===true){
            // console.log("show")
            $("#loading",top.window.document).show();
        }else{
            // console.log("hidden")
            $("#loading",top.window.document).hide();
        }
    }
